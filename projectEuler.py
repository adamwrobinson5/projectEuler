...
# # 8 5 9 3
# # That is, 3 + 7 + 4 + 9 = 23.
# # Find the maximum total from top to bottom of the triangle found in Problem67Triangle.txt
...
with open('Problem67Triangle.txt') as f:        # Reading in the 100 line triangle from text file
    for line in f:
        triangle = line.split('\n')
        triArray.append(map(int,line.split(' ')))
while j > -1:                                   # Used the same algorithm as problem 18
    for i in range(0, len(triArray[j])):
        if triArray[j+1][i] > triArray[j+1][i+1]:
            triArray[j][i] += triArray[j+1][i]
        else:
